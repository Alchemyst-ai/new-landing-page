...
export function Header() {
  ...
  return (
    <div className="fixed top-4 left-0 right-0 z-50 flex justify-center lg:px-4">
      <nav
        ref={navRef}
        className="bg-black/30 backdrop-blur-md border border-white/10 flex justify-between items-center w-[95%] sm:w-[90%] sm:rounded-xl px-4 sm:px-6 py-3 sm:py-4"
      >
        {/* Alchemyst Logo */}
        ...
        {mounted && (
          <AnimatePresence>
            {isOpen && (
              <motion.div
                className="md:hidden fixed top-[70px] inset-x-4 bg-black/90 backdrop-blur-md border border-white/10 rounded-xl shadow-lg flex flex-col items-start p-6 space-y-4 overflow-y-auto max-h-[calc(100vh-100px)]"
                initial={{ opacity: 0, y: -20 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -20 }}
                transition={{ duration: 0.3 }}
