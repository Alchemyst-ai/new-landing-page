...
export default function ChatPlayground() {
  ...
  const { messages, sendMessage, status, stop, setMessages } = useChat<UIMessage>({
    transport: new DefaultChatTransport({
      api: `/api/v1/chat`, body: () => ({
        chatId: sessionStorage.getItem('currentChatId'),
        apiKeyUser : localStorage.getItem('api-key-user')
      })
    }),
    onData: (part) => {
  ...
  useEffect(() => {
    const fetchGroups = async () => {
      try {
        const res = await fetch("/api/v1/groups/view")
        if (!res.ok) {
          throw new Error("Failed to fetch group names")
        }
        const data = await res.json()
        console.log("[v0] Fetched groups:", data.groups)
        ...
        console.error("[v0] Error fetching group names:", err)
  ...
  useEffect(() => {
    const loadChatHistories = async () => {
      try {
        const res = await fetch(`/api/v1/chat/history/fetch`)
        if (!res.ok) throw new Error("Failed to load chat histories")

        const data = await res.json();

        // console.log("Data recieved from chat History : ", data);
  ...
  const handleSelectSession = async (sessionId: string) => {
    ...
    try {
      ...
      // console.log("Data recieved from chat ID : ", data);
      setMessages(history);
  ...
  async function createNewchat() {
    ...
    const saveChatResponse = await fetch(`/api/v1/chat/new/create`, {
      method: 'GET',
    });
    ...
    <div className="flex max-w-auto h-screen bg-background">
      <div className={`flex flex-1 flex-col ${isHistoryOpen ? 'flex-1' : ''}`}>
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          {loadingChat && (
            <div className="h-screen inset-0 z-50 flex items-center justify-center bg-background/50 backdrop-blur-sm transition-all">
          ...
          {!apiKey && <LockedInputOverlay onUnlock={() => setIsModalOpen(true)} />}
          {!isChatEmpty && !loadingChat? (
            <>
              <ChatMessages messages={messages} isStreaming={status === "streaming"} />
