...
<nav className="bg-card/30 backdrop-blur-md border border-border rounded-xl flex justify-between items-center w-[70%] px-6 py-2">
         ...
         <div className="hidden md:flex items-center space-x-6 relative">
           ...
           >
             <button
               className="relative text-muted-foreground hover:text-foreground transition-colors duration-200 text-sm group flex items-center"
               aria-haspopup="true"
             ...
             {activeMenu === "compare" && (
               <div
                 className="absolute top-full left-0 mt-2 w-64 bg-card/90 backdrop-blur-md border border-border rounded-lg shadow-lg p-2"
                 style={{ zIndex: 50 }}
               >
                 {COMPARE_LINKS.map((link) => (
                   <Link
                     key={link.href}
                     href={link.href}
                     className="block px-3 py-2 text-sm text-muted-foreground hover:text-foreground hover:bg-white/5 rounded-md transition-colors"
                   >
                     {link.label}
                   </Link>
                 ))}
                 <Link
                   href="/compare"
                   className="block px-3 py-2 text-sm text-amber-500 hover:text-amber-400 hover:bg-white/5 rounded-md transition-colors border-t border-white/10 mt-1 pt-2"
                 >
                   See all comparisons
                 </Link>
               </div>
