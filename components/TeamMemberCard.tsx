...
const TeamMemberCard: React.FC<TeamMemberCardProps> = ({ member }) => {
  return (
    <div className="flex flex-col items-center justify-center p-4">
      <div className="relative flex flex-col items-end justify-end h-[37vh] overflow-hidden rounded-lg bg-gradient-to-b from-[#04022eed] to-[#353B92] group">
...
const HoverOverlay: React.FC<{ member: Team }> = ({ member }) => (
  <div className="absolute bg-grid inset-0 flex flex-col items-center justify-center bg-[#080419] opacity-0 group-hover:opacity-100 transition-opacity duration-300 p-3">
    <h2 className="text-2xl text-[#cecec5]">{member.name}</h2>
    <p className="text-lg text-[#cecec5]">{member.role}</p>
    <p className="mt-2 text-center text-sm text-[#97978d]">
      {member.achievement}
    </p>
...
const BottomOverlay: React.FC<{ member: Team }> = ({ member }) => (
  <div className="absolute bottom-0 left-0 flex flex-col items-start justify-end w-full h-[38vh] p-4 bg-gradient-to-b from-[#00000038] via-[#00000010] to-[#000000c3] bg-opacity-50 transition-transform duration-300 transform group-hover:-translate-y-full">
    <h2 className="text-2xl text-[#cecec5]">{member.name}</h2>
    <p className="text-lg text-[#99998a]">{member.role}</p>
  </div>
);
