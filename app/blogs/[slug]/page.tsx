...
import { fetchBySlug, notion } from '@/app/lib/notion';
...
export default function Blog({params} : BlogParams) {
    ...
    const [contentBlocks, setContentBlocks] = useState<BlockObjectResponse[]>([]);
    ...
    <div>
      ...
      {contentBlocks.map((block, index) => {
        switch (block.type) {
          ...
          case "quote":
            return (
              <blockquote key={block.id} className="border-l-4 border-blue-500 pl-4 italic text-lg text-[#ff9933]">
            ...
            <ul key={block.id} style={{
              ...
              {/* <li>dkfkjvkjn</li> */}
                {block.bulleted_list_item.rich_text.map((text, idx) => (
                  <li key={idx}>
                    {'text' in text ? text.text.content : ''}
                  </li>
                ))}
            </ul>
            );
